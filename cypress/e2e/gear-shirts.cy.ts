describe(
  "Gear - Shirts tab tests",
  () => {
    it(
      "Tests anchor.",
      () => {
        cy.visit("/gear/?tab=shirts");

        cy.get("[aria-selected='true']")
          .should("have.id", "shirts");
      }
    );
    it("Tests pagination.", () => { cy.testPagination(); });
    it("Tests Name text box filter.", () => { cy.testTextBoxFilter(0, "Name", "Curry"); });
    it("Tests Lvl Avail. dropdown filter.", () => { cy.testSingularSelectDropdown(1, "Lvl Avail.", "7", "equal", "Any"); });
    it("Tests Brand dropdown filter.", () => { cy.testSingularSelectDropdown(2, "Brand", "SLAM", "equal", "Any"); });
    it("Tests Style dropdown filter.", () => { cy.testSingularSelectDropdown(3, "Style", "Jersey", "equal", "Any"); });
